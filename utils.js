...
function getDiscountRate(product) {
  // 請實作此函式
  // 提示：折扣率 = (price / origin_price) * 10
  const discountRate = (product.price / product.origin_price) * 10;
  return `${Math.round(discountRate)}折`;
}
...
function validateOrderUser(data) {
  ...
  if (!data.tel || !data.tel.match(/^09\d{8}$/)) {
  ...
  if (
    !data.payment ||
    !["ATM", "Credit Card", "Apple Pay"].includes(data.payment)
  ) {
